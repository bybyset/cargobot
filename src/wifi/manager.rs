...
impl WifiManager {
    pub fn new<M: WifiModemPeripheral + 'static>(modem: M) -> Result<Self> {
        // 先获取NVS分区
        let nvs_partition = EspNvsPartition::<NvsDefault>::take().map_err(|e| {
        ...
        let state = wifi_state_from_nvs(&nvs);
        Ok(Self {
            state,
    ...
    fn start_provisioning(&mut self) -> Result<()> {
        log::info!("启动配网模式...");
        // start_softap_mode(&mut self.wifi, &self.config)?;
...
fn wifi_state_from_nvs(nvs: &NvsStorage) -> WifiState {
    if nvs.has_wifi_credentials() {
