...
use crate::{audio::{MicrophoneReader, MicrophoneService, PcmFormat, SpeakerService}};

use super::config::{API_RESOURCE_ID, AUDIO_CHUNK_SIZE, RuntimeConfig, WS_URL};
use super::net_client::NetClient;
use super::protocol::{Message, MsgType, generate_session_id};
use super::request_payloads::{self, SayHelloPayload};
use super::DoubaoError;
...
impl SessionContext {
    ...
    pub fn start_microphone_chat(
        &mut self,
        microphone: Arc<MicrophoneService>,
    ) -> Result<(), DoubaoError> {
...
impl RtService {
    pub fn new(sound: Arc<SpeakerService>, microphone: Arc<MicrophoneService>, config: RuntimeConfig) -> Self {
        // 事件通道
...
fn handle_audio_only_server_message(message: Message, sound: Arc<SpeakerService>, pcm_format: PcmFormat) {
    ...
    if payload_len > 0 {
        ...
        let _ = sound.play_audio_data(&audio, pcm_format);
    }
}
